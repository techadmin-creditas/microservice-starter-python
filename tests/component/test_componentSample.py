import pytest
import sys
sys.path.append('/home/krantitech/python/.env/microservice-starter-python')
import asyncio
from src.controllers.health import health
from src.controllers.word_controller import get_word

#validates the status code and responseOutput for health
def test_health():
    #act
    response = health()
...
def test_greetings_newWord():
    ...
    word="One"
    #act
    response = asyncio.run(get_word(word))
    #assert response=='qwe'
    status_code=response[1]
